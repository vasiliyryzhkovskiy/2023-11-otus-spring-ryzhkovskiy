...
public class InitMongoDBDataChangeLog {
    ...
    public void initBooks(BookRepository repository) {
        Book book1 = repository.save(new Book("BookTitle_1", authors.get(0), genres.get(0)));
        Book book2 = repository.save(new Book("BookTitle_2", authors.get(1), genres.get(1)));
        Book book3 = repository.save(new Book("BookTitle_3", authors.get(2), genres.get(2)));
        books = List.of(book1, book2, book3);
    }

    @ChangeSet(order = "005", id = "initComments", author = "vasiliy", runAlways = true)
    public void initComments(CommentRepository repository) {
        repository.save(new Comment(null, "Comment_1", books.get(0)));
        repository.save(new Comment(null, "Comment_2", books.get(1)));
        repository.save(new Comment(null, "Comment_3", books.get(2)));
    }
}